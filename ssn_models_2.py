...
class SSN(torch.nn.Module):
    ...
    def _prepare_ssn(self, num_class, stpp_cfg):
        # feature_dim = getattr(self, self.base_model.last_layer_name).in_features
        feature_dim = 1024
        self.base_out_fc = nn.Linear(self.base_model.out_dim, feature_dim)
        nn.init.normal(self.base_out_fc.weight.data, 0, 0.001)
        nn.init.constant(self.base_out_fc.bias.data, 0)
    ...
    def _prepare_base_model(self, base_model):
        if 'resnet' in base_model or 'vgg' in base_model:
            ...
            self.base_model = P3D199(pretrained=True, num_classes=400)
            self.input_size = 160
    ...
    def train(self, mode=True):
        ...
        # freeze all params in base model
        for m in self.base_model.modules():
            m.eval()
    ...
    def get_optim_policies(self):
        ...
        for m in self.modules():
            if isinstance(m, torch.nn.Conv2d) or isinstance(m, torch.nn.Conv1d) or isinstance(m, torch.nn.Conv3d):
                ...
                if conv_cnt == 1:
                    ...
                    later_conv_weight.append(ps[0])
                    if len(ps) == 2:
                        later_conv_bias.append(ps[1])
        ...
        return [
            {'params': first_conv_weight, 'lr_mult': 0, 'decay_mult': 0,
             'name': "first_conv_weight"},
            {'params': first_conv_bias, 'lr_mult': 0, 'decay_mult': 0,
             'name': "first_conv_bias"},
            {'params': later_conv_weight, 'lr_mult': 0, 'decay_mult': 0,
             'name': "later_conv_weight"},
            {'params': later_conv_bias, 'lr_mult': 0, 'decay_mult': 0,
             'name': "later_conv_bias"},
    ...
    def train_forward(self, input, aug_scaling, target, reg_target, prop_type):
        ...
        input  = input.view(-1, 16, 3, input.size()[2], input.size()[3])
        input = torch.transpose(input, 1, 2)

        put = torch.zeros((input.size()[0]//7, input.size()[1], input.size()[2], input.size()[3], input.size()[4]))

        base_out_list = []
        cnt = 0
        for i in range(0, 7):
            put = input[cnt:cnt+put.size()[0], :, :, :, :]
            base_out_list.append(self.base_model(put))
            cnt += put.size()[0]
        base_out = torch.cat(base_out_list)

        base_out = self.base_out_fc(base_out)
    ...
    @property
    def scale_size(self):
        return self.input_size * 256 // 160
