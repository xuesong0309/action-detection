...
class P3D(nn.Module):
    def __init__(self, block, layers, modality='RGB',
        ...
        self.cnt=0
        self.relu = nn.ReLU(inplace=True)
        self.maxpool = nn.MaxPool3d(kernel_size=(2, 3, 3), stride=2, padding=0, ceil_mode=True)       # pooling layer for conv1.
        self.maxpool_2 = nn.MaxPool3d(kernel_size=(2,1,1),padding=0,stride=(2,1,1), ceil_mode=True)   # pooling layer for res2, 3, 4.
...
def get_transfer_weights(model, weights):
...
def P3D199(pretrained=False,modality='RGB',**kwargs):
    ...
    if pretrained==True:
        ...
        weights = get_transfer_weights(model, weights)
